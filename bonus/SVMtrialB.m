...
for j = 1:N
    for k = 1:j
        d = x(j,:) - xT(k,:);
        H(j,k) = y(j)*y(k)*exp(-(d*d')/(2*(sigma^2)));     % Create kernel matrix
...
temp = zeros(size(nb));
for j = 1:length(nb)
    temp(j) = 1/y(nb(j)) - func(x(nb(j),:),xT,y,a,0,sigma,sv);
end
...
for jX = 1:length(X)
    for jY = 1:length(Y)
        xi = [X(jX,jY) Y(jX,jY)];
        Z(jX,jY) = func(xi,xT,y,a,b,sigma,sv);     % Solve for heights, Z
    end
end
...
zp = zeros(size(y));
for j = 1:N
    zp(j) = func(xT(j,:),xT,y,a,b,sigma,sv);       % Project training data
end
